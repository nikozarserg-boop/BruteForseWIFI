...
namespace NativeWifi
{
	...
	{
		public class WlanConnectionResult
		{
			public bool Success { get; set; }
			public Wlan.WlanReasonCode ReasonCode { get; set; }
		...
		public class WlanInterface
		{
			...
			public WlanClient.WlanConnectionResult ConnectSynchronouslyWithReason(Wlan.WlanConnectionMode connectionMode, Wlan.Dot11BssType bssType, string profile, int connectTimeout)
			{
				queueEvents = true;
				try
				{
						...
						lock (eventQueue)
						{
							while (eventQueue.Count != 0)
							{
								...
								if (e is WlanConnectionNotificationEventData)
								{
									...
									// Проверка условий для указания успеха или ошибки.
											...
											case Wlan.WlanNotificationCodeAcm.ConnectionComplete:
												if (wlanConnectionData.connNotifyData.profileName == profile)
												{
															...
															if (netInterface != null)
															{
																...
																if (hasValidIP)
																{
																	...
																	try
																	{
																		...
																		{
																			var gateway = gateways[0].Address;
																			using (System.Net.NetworkInformation.Ping ping = new System.Net.NetworkInformation.Ping())
																			{
																				var pingReply = ping.Send(gateway, 7000); // Таймаут
																				if (pingReply.Status == System.Net.NetworkInformation.IPStatus.Success)
																				{
																					return new WlanClient.WlanConnectionResult 
																					{ 
																						Success = true, 
																						ReasonCode = Wlan.WlanReasonCode.Success,
																						ReasonMessage = "Успешное подключение - доступен шлюз"
																					};
																				}
																			}
																		}
																	...
																	{ 
																		Success = false, 
																		ReasonCode = Wlan.WlanReasonCode.UNKNOWN,
																		ReasonMessage = "Получен IP но шлюз недоступен"
																	};
																	}
																	else
																	{
																	...
																	{ 
																		Success = false, 
																		ReasonCode = Wlan.WlanReasonCode.UNKNOWN,
																		ReasonMessage = "IP адрес не получен"
																	};
																	}
																	}
																	else
																	...
																	Success = false, 
																	ReasonCode = Wlan.WlanReasonCode.UNKNOWN,
																	ReasonMessage = "Сетевой интерфейс недоступен"
																	};
																	}
																	}
																	catch
																	...
																	Success = false, 
																	ReasonCode = Wlan.WlanReasonCode.UNKNOWN,
																	ReasonMessage = "Ошибка при проверке подключения"
																	};
														}
													}
													else
													{
														...
														{ 
															Success = false, 
															ReasonCode = wlanConnectionData.connNotifyData.wlanReasonCode,
															ReasonMessage = $"Ошибка подключения: {wlanConnectionData.connNotifyData.wlanReasonCode}"
														};
													}
												}
												break;
											case Wlan.WlanNotificationCodeAcm.ConnectionAttemptFail:
												if (wlanConnectionData.connNotifyData.profileName == profile)
												{
													...
													{ 
														Success = false, 
														ReasonCode = wlanConnectionData.connNotifyData.wlanReasonCode,
														ReasonMessage = $"Попытка подключения провалилась: {wlanConnectionData.connNotifyData.wlanReasonCode}"
													};
												}
												break;
											case Wlan.WlanNotificationCodeAcm.Disconnected:
												if (wlanConnectionData.connNotifyData.profileName == profile)
												{
													...
													{ 
														Success = false, 
														ReasonCode = wlanConnectionData.connNotifyData.wlanReasonCode,
														ReasonMessage = "Отключено от сети"
													};
												}
												break;
										}
				...
				{ 
					Success = false, 
					ReasonCode = Wlan.WlanReasonCode.UNKNOWN,
					ReasonMessage = "Таймаут истёк - нет ответа от сети"
				};
			}
			...
			{
				var result = ConnectSynchronouslyWithReason(connectionMode, bssType, profile, connectTimeout);
		...
		{
			StringBuilder sb = new StringBuilder(1024); // Размер 1024 произвольный; документы WlanReasonCodeToString не указывают рекомендуемый размер
			Wlan.ThrowIfError(
